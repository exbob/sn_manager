...
- `sn TEXT PRIMARY KEY`
- 解析列：`version`, `product_model`, `hw_batch`, `factory`, `market`
- `prod_year` INTEGER（公历年，如 2026）、`prod_month` INTEGER（1–12）、`prod_day` INTEGER（1–31）— **存日历数值**，编码进 SN 时再转为两位年与月日字符
- `seq` INTEGER — 当日序号 0–4095（对应 `000`–`FFF`）
- `status TEXT NOT NULL` — `unused` | `used` | `void`
- `created_at`, `updated_at` — 统一用 UTC ISO-8601 文本
- **UNIQUE** `(product_model, hw_batch, prod_year, prod_month, prod_day, seq)`

索引建议：`(product_model, hw_batch, prod_year, prod_month, prod_day)`、`status`、按筛选需要补齐。

日期规则：生成对话框用真实日历选择；拒绝不存在的日期（如 2 月 31 日）。字母输入统一转大写后再校验/入库。
...
- 「全选」= 选中**当前结果表中的全部行**（非全库）
- 「改状态」对选中行三态任意更新，写 `updated_at`
- 「作废」仍可被筛选查出；是否允许导出见实现约定：允许对选中行导出（含作废），由操作员自负

### 5.3 导出

- 仅导出**当前选中行**；无选中时「改状态」「导出」均禁用
- 导出对话框：选择 Excel 或烧写 txt
- Excel：选中行 → 单文件 `.xlsx`（SN、各解析字段、状态、时间戳）
- 烧写：目录下 `sn_<设备序列号>.txt`，内容仅一行 SN；**同名文件已存在则覆盖**
...
| ---- | ------ |
| 查询 | 按左侧条件查库，填充右侧表 |
| 生成 | 模态对话框填条件与 N → 确认则生成、关窗、右侧**仅显示本批**新 SN（默认选中）；取消则不生成 |
| 主数据 | 模态对话框内编辑，**确认才写入数据库**；取消则放弃本次对话框内全部修改 |
| 改状态 | 右下角、导出左侧；须先选中行 → 选择目标状态并应用（无选中时禁用） |
| 导出 | 右下角；须先选中行 → 对话框选 Excel / 烧写 txt（烧写可勾选标为已使用）；无选中时禁用 |
| 全选 | 选中当前结果表全部行 |
...
- 非法长度/字符/日期码拒绝；非法公历日期拒绝
- 同维度连续 seq；作废不回收；触顶整批无写入
- N=1 与 N>1；仅选中行可导出；同名烧写文件覆盖；导出后标 used 仅全成功后生效
...
- 多机共享序号池 / 服务端
- 同一台机器多进程同时写库的协调（依赖 SQLite 默认锁；文档注明勿双开写）
- 用户权限与登录
...
- 只读安装目录等导致无法在可执行文件旁建库时的降级策略（需在安装说明中写清）
- ORM（如 SQLAlchemy）vs 手写 SQL
- Excel 库（如 openpyxl）
- 两位年份的世纪约定：首期按 **2000+YY**（如 26→2026）；不支持跨世纪歧义扩展
